...
struct ResponseHeader {
    double cpu_time;  // tiempo de ejecución en segundos (omp_get_wtime)
...
// ---------------------------------------------------------
void ejecutar_kmeans_paralelo(int K, int max_iters, int socket_cliente) {
    int dim = 0;
...
int main() {
    ...
    cout << "Servidor K-Means (PARALELO) escuchando en puerto " << PORT << endl;
    cout << "Threads disponibles (OpenMP): " << omp_get_max_threads() << endl;
    ...
    while (true) {
        ...
        // Ejecutar K-Means paralelo y enviar resultados al cliente
        ejecutar_kmeans_paralelo(req.k, req.max_iters, new_socket);
